#!/usr/bin/env bash
# install-raspberrypi.sh — Deploy Trunk8 on Raspberry Pi (Raspberry Pi OS)
set -euo pipefail
...
if ! grep -qi 'raspberry\|debian' /etc/os-release 2>/dev/null; then
    warn "This script is designed for Raspberry Pi OS (Debian). Proceed at your own risk."
fi

ARCH=$(uname -m)
if [[ "$ARCH" != arm* && "$ARCH" != aarch64 ]]; then
    warn "Detected architecture '${ARCH}' — this script is intended for ARM (Raspberry Pi)."
fi

# ---------- prompt for config ----------
read -rp "Domain name (leave blank for LAN-only access): " DOMAIN
...
# ---------- system packages ----------
info "Installing system packages (this may take a moment on Pi)..."
apt-get update -qq
apt-get install -y -qq python3 python3-venv nginx git curl > /dev/null
...
# ---------- install app deps ----------
info "Installing application dependencies (may be slow on Pi — be patient)..."
cd "$APP_DIR"
...
if [[ -n "$DOMAIN" ]]; then
    ...
    info "No domain provided — skipping SSL (typical for LAN-only Pi setups)."
fi
...
if [[ -n "$DOMAIN" ]]; then
    ...
    echo "  URL:      http://${HOSTNAME_IP:-<your-pi-ip>}"
fi
...
if [[ -z "$DOMAIN" ]]; then
    echo "  - For LAN access, use http://${HOSTNAME_IP:-<your-pi-ip>}"
    echo "  - For external access, set up port forwarding and a domain"
fi
echo "  - Monitor: journalctl -u trunk8 -f"
echo
